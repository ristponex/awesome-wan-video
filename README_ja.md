...
*最先端のAI動画生成モデルファミリー — オープンソースモデル（2.1/2.2）と最先端商用API（2.5/2.6）。厳選されたツール、プロンプト、ガイド、リソース集。*

[English](./README.md) | [简体中文](./README_zh-CN.md) | [日本語](./README_ja.md) | [한국어](./README_ko.md)

---

🚀 **API経由でWan動画を生成 — $0.03/秒から** 🚀

[![Atlas Cloudで試す](https://img.shields.io/badge/今すぐ試す-Atlas%20Cloud-blue?style=for-the-badge&logo=cloud)](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
...
**Wan**はAlibabaが開発した動画生成モデルシリーズです。Wan 2.1と2.2は**完全オープンソース**（Apache 2.0）でHugging Faceからウェイトをダウンロードしてセルフデプロイ可能ですが、Wan 2.5と2.6は**クローズドソースの商用API**モデルとしてより高度な機能を提供しています。デビューから現在の**VBenchリーダーボード1位**まで、Wanは動画AIの限界を押し広げ続けています。

### なぜWanなのか？

- **🏆 VBenchチャンピオン** — Wan 2.1はVBenchで**86.22%**のスコアを達成し、Sora（84.28%）、Luma（83.61%）を含む全モデルを上回る
- **🔓 オープンソース (2.1/2.2)** — Sora、Kling、Runwayとは異なり、Wan 2.1/2.2は完全オープンソース（Apache 2.0）でHugging Faceからウェイトをダウンロードしてセルフデプロイ可能
...
```
Wan 1.0 (2024 Q3)
  └── Wan 2.1 (2024 Q4) — VBench 1位、14B & 1.3B [オープンソース, Apache 2.0]
       └── Wan 2.2 (2025 Q1) — 品質向上、Spicyバリアント 🌶️ [オープンソース, Apache 2.0]
            ├── Wan 2.2 Spicy — NSFW/無検閲生成 [プロプライエタリ, Atlas Cloud LoRA]
            └── Wan 2.5 (2025 Q2) — オーディオ同期、1080p、10秒 [クローズドソース, APIのみ]
                 └── Wan 2.6 (2025 Q3) — マルチショット、15秒、キャラクター安定性 [クローズドソース, APIのみ]
                      └── Wan 2.7 (近日公開！) — 🔜 Atlas Cloudで先行提供
```

### クローズドソースモデルとの主な違い

| 機能 | Wan 2.1/2.2 | Wan 2.5/2.6 | Sora | Kling | Runway Gen-3 |
|------|-------------|-------------|------|-------|---------------|
| オープンソース | ✅ Apache 2.0 | ❌ APIのみ | ❌ | ❌ | ❌ |
| ローカルデプロイ | ✅ | ❌ | ❌ | ❌ | ❌ |
| 最大解像度 | 720p | 1920×1080 | 1920×1080 | 1920×1080 | 1920×1080 |
| 最大長さ | 5秒 | 15秒 | 20秒 | 10秒 | 10秒 |
| ネイティブオーディオ | ❌ | ✅ | ✅ | ❌ | ❌ |
| マルチショット | ❌ | ✅ | ❌ | ❌ | ❌ |
| NSFWサポート | ✅ (Spicy) | ❌ | ❌ | ❌ | ❌ |
| 動画あたりの価格 | セルフホスト | $0.03から | $0.50以上 | $0.30以上 | $0.50以上 |
| コンシューマーGPU | ✅ (1.3B) | N/A | N/A | N/A | N/A |
| ファインチューニング | ✅ LoRA | ❌ | ❌ | ❌ | ❌ |
...
|-----------|-----------|--------|------|-----------|---------------|---------------|------|---------|
| **Wan 2.6** | 14B | 1080p | 最大15秒 | ✅ | ✅ | ❌ (APIのみ) | ❌ | $0.07 |
| **Wan 2.5** | 14B | 1080p | 最大10秒 | ✅ | ❌ | ❌ (APIのみ) | ❌ | より安い |
...
> 🎁 **新規ユーザー特典：** 初回チャージで25%ボーナス（最大$100ボーナス）。[今すぐ登録 →](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
...
> 💡 **プロのティップス：** [Atlas Cloudに登録](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)すると初回チャージで**25%ボーナス**（最大$100ボーナス）。$100+$25ボーナスで最大1,785本のWan Spicy動画またはWan 2.6動画を生成！
...
WanはAlibabaの動画生成モデルシリーズです。Wan 2.1/2.2は**オープンソース（Apache 2.0）**でHugging Faceからウェイトをダウンロードしてセルフデプロイ可能ですが、Wan 2.5/2.6は**クローズドソースの商用API**モデルです。拡散Transformerアーキテクチャを使用し、15億本の動画と100億枚の画像で学習しています。Wan 2.1モデルは**VBenchリーダーボードで1位**を達成し、スコアは86.22%で、Sora（84.28%）とLuma（83.61%）を上回りました。最新版は**Wan 2.6**で、ネイティブオーディオ同期付きの最大15秒のマルチショット1080p動画生成をAPI経由でサポートしています。
...
**Wanの優位点：** 価格、NSFWサポート、機能の豊富さ。Wan 2.1/2.2はオープンソースの柔軟性とローカルデプロイを提供し、Wan 2.5/2.6はAPI経由で最先端の品質を提供します。SeedanceやKlingは特定のユースケースでモーション品質が異なる場合がありますが、Wanの迅速な進化により、長期的な最良の選択です。
...
**予算の推奨：** ローカルデプロイにはRTX 4090、またはハードウェアに投資したくない場合は[Atlas Cloud API](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)を$0.03/動画から利用。
...
[Atlas CloudでWan 2.2 Spicyを試す →](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
...
Sora（$0.50以上/動画）やRunway（$0.50以上/動画）と比較してください。[始める →](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
...
**Wan 2.7**は現在開発中で、近日公開予定です。Atlas CloudはWan 2.7をAPIで提供する最初のプラットフォームの1つになります。[Atlas Cloudに登録](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)して、Wan 2.7ローンチ時に通知を受け取りましょう。
...
| ✅ **オープンソース (2.1/2.2)** | ローカル実行またはAPI利用 |
...
[![Atlas CloudでWanを試す](https://img.shields.io/badge/👉%20Atlas%20Cloudで%20Wanを試す-blue?style=for-the-badge&logo=cloud&logoColor=white)](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
