...
*세계 최고의 AI 비디오 생성 모델 패밀리 — 오픈소스 모델 (2.1/2.2) 및 최첨단 상용 API (2.5/2.6). 엄선된 도구, 프롬프트, 가이드, 리소스 모음.*
...
[![Atlas Cloud에서 사용해보기](https://img.shields.io/badge/지금%20사용해보기-Atlas%20Cloud-blue?style=for-the-badge&logo=cloud)](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
...
**Wan**은 알리바바가 개발한 비디오 생성 모델 시리즈로, AI 업계에 큰 반향을 일으키고 있습니다. Wan 2.1과 2.2는 **완전 오픈소스** (Apache 2.0)로 Hugging Face에서 가중치를 다운로드할 수 있으며, Wan 2.5와 2.6은 고급 기능을 갖춘 **클로즈드소스 상용 API** 모델입니다. 데뷔 이래 **VBench 리더보드 1위**까지, Wan은 비디오 AI의 한계를 지속적으로 확장하고 있습니다.

### 왜 Wan인가?

- **🏆 VBench 챔피언** — Wan 2.1은 VBench에서 **86.22%** 점수를 달성하여 Sora(84.28%), Luma(83.61%) 등 모든 모델을 능가
- **🔓 오픈소스 (2.1/2.2)** — Sora, Kling, Runway와 달리, Wan 2.1/2.2는 완전 오픈소스(Apache 2.0)로 Hugging Face에서 가중치 다운로드 및 자체 배포 가능
...
```
Wan 1.0 (2024 Q3)
  └── Wan 2.1 (2024 Q4) — VBench 1위, 14B & 1.3B [오픈소스, Apache 2.0]
       └── Wan 2.2 (2025 Q1) — 품질 향상, Spicy 변형 🌶️ [오픈소스, Apache 2.0]
            ├── Wan 2.2 Spicy — NSFW/무검열 생성 [독점, Atlas Cloud LoRA]
            └── Wan 2.5 (2025 Q2) — 오디오 동기화, 1080p, 10초 [클로즈드소스, API 전용]
                 └── Wan 2.6 (2025 Q3) — 멀티샷, 15초, 캐릭터 안정성 [클로즈드소스, API 전용]
                      └── Wan 2.7 (곧 출시!) — 🔜 Atlas Cloud에서 최초 제공
```

### 클로즈드 소스 모델과의 주요 차이점

| 기능 | Wan 2.1/2.2 | Wan 2.5/2.6 | Sora | Kling | Runway Gen-3 |
|------|-------------|-------------|------|-------|---------------|
| 오픈소스 | ✅ Apache 2.0 | ❌ API 전용 | ❌ | ❌ | ❌ |
| 로컬 배포 | ✅ | ❌ | ❌ | ❌ | ❌ |
| 최대 해상도 | 720p | 1920×1080 | 1920×1080 | 1920×1080 | 1920×1080 |
| 최대 길이 | 5초 | 15초 | 20초 | 10초 | 10초 |
| 네이티브 오디오 | ❌ | ✅ | ✅ | ❌ | ❌ |
| 멀티샷 | ❌ | ✅ | ❌ | ❌ | ❌ |
| NSFW 지원 | ✅ (Spicy) | ❌ | ❌ | ❌ | ❌ |
| 비디오당 가격 | 자체 호스팅 | $0.03부터 | $0.50+ | $0.30+ | $0.50+ |
| 소비자용 GPU | ✅ (1.3B) | N/A | N/A | N/A | N/A |
| 파인튜닝 | ✅ LoRA | ❌ | ❌ | ❌ | ❌ |
...
|------|---------|--------|------|--------|--------|---------|------|---------|
| **Wan 2.6** | 14B | 1080p | 최대 15초 | ✅ | ✅ | ❌ (API 전용) | ❌ | $0.07 |
| **Wan 2.5** | 14B | 1080p | 최대 10초 | ✅ | ❌ | ❌ (API 전용) | ❌ | 더 낮음 |
...
> 🎁 **신규 사용자 혜택:** 첫 충전 시 25% 보너스 (최대 $100 보너스). [지금 가입 →](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
...
> 💡 **프로 팁:** [Atlas Cloud에 가입](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)하면 첫 충전 시 **25% 보너스** (최대 $100 보너스). $100+$25 보너스로 최대 1,785개의 Wan Spicy 비디오 또는 Wan 2.6 비디오 생성 가능!
...
Wan은 알리바바의 비디오 생성 모델 시리즈입니다. Wan 2.1과 2.2는 **오픈소스 (Apache 2.0)**로 Hugging Face에서 가중치를 다운로드하여 자체 배포할 수 있으며, Wan 2.5와 2.6은 **클로즈드소스 상용 API** 모델입니다. 확산 트랜스포머 아키텍처를 사용하며, 15억 개의 비디오와 100억 개의 이미지로 학습되었습니다. Wan 2.1 모델은 **VBench 리더보드에서 1위**를 달성했으며, 86.22%의 점수로 Sora(84.28%)와 Luma(83.61%)를 능가했습니다. 최신 버전은 **Wan 2.6**으로, 네이티브 오디오 동기화와 함께 최대 15초의 멀티샷 1080p 비디오 생성을 지원하며, API를 통해 이용 가능합니다.
...
**Wan의 강점:** 가격, NSFW 지원, 풍부한 기능. Wan 2.1/2.2는 오픈소스 유연성과 로컬 배포를 제공하며, Wan 2.5/2.6은 API를 통해 최첨단 품질을 제공합니다. Seedance와 Kling은 특정 사용 사례에서 모션 품질이 다를 수 있지만, Wan의 빠른 발전으로 장기적으로 최선의 선택입니다.
...
**예산 추천:** 로컬 배포에는 RTX 4090, 또는 하드웨어에 투자하고 싶지 않다면 [Atlas Cloud API](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)를 비디오당 $0.03부터 이용하세요.
...
[Atlas Cloud에서 Wan 2.2 Spicy 사용해보기 →](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
...
Sora($0.50+/비디오)나 Runway($0.50+/비디오)와 비교해보세요. [시작하기 →](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
...
**Wan 2.7**은 현재 개발 중이며 곧 출시될 예정입니다. Atlas Cloud는 Wan 2.7을 API로 제공하는 최초의 플랫폼 중 하나가 될 것입니다. [Atlas Cloud에 가입](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)하여 Wan 2.7 런칭 시 알림을 받으세요.
...
[![Atlas Cloud에서 Wan 사용해보기](https://img.shields.io/badge/👉%20Atlas%20Cloud에서%20Wan%20사용해보기-blue?style=for-the-badge&logo=cloud&logoColor=white)](https://www.atlascloud.ai?ref=JPM683&utm_source=github&utm_campaign=awesome-wan-video)
